...
export const getJustification = async (
    applicationId: string,
    signal?: AbortSignal
   ) => {
    const response = await fetch(
      `${API_ENDPOINTS.GET_JUSTIFICATION}?application_id=${applicationId}`,
      {
        method: 'GET',
        signal,
      }
...
export const fetchJustification = createAsyncThunk(
  'justification/fetchJustification',
  async (applicationId: string, { rejectWithValue }) => {
    try {
      const data = await getJustification(applicationId);
      return data;
    } catch (error: any) {
